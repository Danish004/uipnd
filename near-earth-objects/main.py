...
command shell that can repeatedly execute `inspect` and
`query` commands without
having to wait to reload the database each time. However,
...
# The current time, for use with the kill-on-change feature
...
class NEOShell(cmd.Cmd):
    ...
    def do_query(self, arg):
        ...
        This command behaves the same as the `query` subcommand
        from the command
    ...
    def precmd(self, line):
        ...
        path = PROJECT_ROOT.glob("*.py")
        changed = [f for f in path if f.stat().st_mtime > _START]
        if changed:
            string = ", ".join(str(f.relative_to(PROJECT_ROOT)) for f in changed)
            print(
